...
def test_eval_unify():
    ...
    result = evaluate(parse("(unify INR[1] {+F})"), [{"G": "+"}], [], ALPHABET)
    assert result == {"F": "+", "G": "+"}


def test_eval_unify_blocked():
    # B = {+F, -G}; unify with {+G} blocked because -G (opposite) IS present → stays B
    result = evaluate(parse("(unify INR[1] {+G})"), [B], [], ALPHABET)
    assert result == B


def test_eval_subtract():
    # A = {+F, +G}; subtract {+F} → {+G}
    result = evaluate(parse("(subtract INR[1] {+F})"), [A], [], ALPHABET)
    assert result == {"G": "+"}


def test_eval_project():
    # A = {+F, +G}; proj (G) → {+G}
    result = evaluate(parse("(proj INR[1] (G))"), [A], [], ALPHABET)
    assert result == {"G": "+"}
...
def test_eval_in_class_true():
    assert evaluate(parse("(in? INR[1] [{+F}])"), [A], [], ALPHABET) is True
...
# ---------------------------------------------------------------------------
# evaluate — implicit concat (word-level output)
# ---------------------------------------------------------------------------


def test_eval_concat_identity():
    # (INR) — returns the whole INR word unchanged
    assert evaluate(parse("(INR)"), [A, B], [], ALPHABET) == [A, B]


def test_eval_concat_metathesis():
    # swap two segments
    result = evaluate(parse("(INR[2] INR[1])"), [A, B], [], ALPHABET)
    assert result == [B, A]
...
def test_eval_concat_epenthesis():
    ...
    result = evaluate(
        parse("(INR[1] {+F -G} INR[2])"), [A, C], [], ALPHABET
    )
...
def test_eval_concat_symbol_epenthesis():
    result = evaluate(
        parse("(INR[1] &D INR[2])"), [A, C], [], ALPHABET
    )
    assert result == [A, D, C]
...
ASSIMILATION_2SEG = Rule(
    ...
    Trm=[["+F"]],
    Dir="L",
    Out="((unify (subtract INR[1] {-F}) {+F}) (unify (subtract INR[2] {-F}) {+F}))",
)
