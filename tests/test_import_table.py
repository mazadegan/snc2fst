...
def test_import_table_csv_example(tmp_path: Path) -> None:
    csv_content = ", a, b, c\nvoice, +, -, 0\nnasal, 0, +, -\n"
    table_path = tmp_path / "table.csv"
    table_path.write_text(csv_content, encoding="utf-8")

    runner = CliRunner()
    result = runner.invoke(app, ["import-table", str(table_path)])
...
def test_import_table_tsv_example(tmp_path: Path) -> None:
    tsv_content = "\ta\tb\tc\nvoice\t+\t-\t0\nnasal\t0\t+\t-\n"
